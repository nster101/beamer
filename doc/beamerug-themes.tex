...
These themes share a small engine, |beamercorporatekit|, which a colour theme loads to derive a complete beamer colour set from a two-anchor palette using \declare{|\setcorporatepalette|}|{brand=...,accent=...,ink=...,gray=...,paper=...,alert=...}| (with |titlestyle=light| or |dark|), and which a font theme uses to load a sans typeface with a graceful fallback via \declare{|\setcorporatesansfont|}. Building a new corporate theme on this engine therefore amounts to declaring a handful of raw colours and a font.
