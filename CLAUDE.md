...
This fork is developed by a **five-agent Claude Code crew** running as independent
sessions with **no shared memory**; they coordinate **only through git**. Load (or
reload, in any fresh window) an agent's persona with its slash command:
...
- **Coordination PRs are small and separate:** they touch only your own log file
  ...
  `coordination/`.
- **Branches:** `<agent>/<issue#>-<slug>` (e.g. `euclid/17-corridor-onto-kit`).
...
Full protocol and entry format: **`coordination/README.md`**. The crew is also
packaged as a reusable plugin for other projects — see
**`.claude/agent-harness/README.md`**.
